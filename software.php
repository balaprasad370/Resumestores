...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
  if(isset($_POST['sub'])){
    ...
    $starskill = $_POST['star'];
        ...
        $updatetrue = "UPDATE software set softwareskill='$skillimp', softwarestar= '$starimp' where sessionid='123456789';";
        $resulttrue = mysqli_query($conn,$updatetrue);
    }else{
      $updatefalse = "UPDATE software set softwareskill='', softwarestar= '' where sessionid='123456789';";
...
function fetch(){
   ...
   <?php
   $fet = "SELECT * from software where sessionid='123456789';";
   $fetchresult = mysqli_query($conn,$fet);
   while($row = mysqli_fetch_array($fetchresult)){
       $skill1 = $row['softwareskill'];
       $star1 = $row['softwarestar'];
